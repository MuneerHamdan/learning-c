...
GS1 prefix: 978 Group identifier: 0
Publisher code: 393
Item number: 97950
Check digit: 3

Note: The number of digits in each group may vary; you can’t assume that groups have the
lengths shown in this example. Test your program with actual ISBN values (usually found on the back cover of a book and on the copyright page).
*/
#include <stdio.h>
int main(void) {
  int gs1, identifier, publisher, item, check;
  printf("Enter ISBN: ");
  fscanf(stdin, "%d-%d-%d-%d-%d", &gs1, &identifier, &publisher, &item, &check);
  printf("GS1 prefix: %d Group identifier: %d\nPublisher code: %d\nItem number: %d\nCheck digit: %d\n", gs1, identifier, publisher, item, check);
  return 0;
}
